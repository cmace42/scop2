...
layout(location = 3) in vec3 vertexPostion_modelSpace2;
// Notez que le "1" ici correspond au "1" dans glVertexAttribPointer 
layout(location = 4) in vec3 vertexColor2;
...
void main(){
	...
	vec4 v2 = vec4(vertexPostion_modelSpace2, 1); // Transforme un vecteur 4D homogène, vous vous souvenez ?
	gl_Position = MVP * v;
