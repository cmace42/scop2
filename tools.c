...
/*   Updated: 2022/03/01 13:10:41 by cmace            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_mat4	initPerspective(float fov, float zNear, float zFar)
{
	...
	ar = 4.0f / 3.0f;
	tanHalfFOV = tanf(toRadian(fov / 2.0));
	return ((t_mat4){
		.m = {
...
t_mat4	lookAt(t_vec3 cameraPosition, t_vec3 cameraTarget, t_vec3 upVector)
{
	...
	cameraDir = vec3_normalisation(vec3_sub(cameraPosition, cameraTarget));
	cameraRight = vec3_normalisation(vec3_cross(upVector, cameraDir));
	cameraUp = vec3_cross(cameraDir, cameraRight);
...
t_mat4	initModelMatrice(t_vec3 angle, t_vec3 whl, t_vec3 pos)
{
	...
	rotate = mat4_mult(mat4_translation((t_vec3){-whl.x + pos.x, -whl.y + pos.y, -whl.z + pos.z}), mat4_identity());
	scale = mat4_identity();
	...
	translation = mat4_translation((t_vec3){-pos.x, -pos.y, -pos.z});
	modelMatrice = mat4_mult(mat4_mult(scale, rotate), translation);
	return (modelMatrice);
}
